...
(function ($) {
    ...
    $.fn.extend({
         ...
         */
        diyUpload: function diyUpload(opt, serverCallBack) {
    ...
    //取消事件;
    function removeLi($li, file_id, webUploader, index) {
        ...
        app.setItem('uploadImgArr', uploadImgArr);
    }
    ...
    //-统计图片数量
    function imgNum() {
        var length = $('.upload-ul li').length;
        // console.log(length)
        $('.uploadImg p span').text(length - 1);
        if (length >= 5) {
            $('.upload-pick').hide();
        } else {
            $('.upload-pick').show();
    ...
    //-上传图片
    var uploadImgArr = [];
    function uploadImg(dataSrc) {
        if (!getUrlParam('customerId')) {
            app.alert('缺少用户id');
            return false;
        } else if (!dataSrc) {
            app.alert('请上传图片');
        ...
        $.ajax({
            ...
            success: function success(res) {
                console.log(res);
                if (res.code == 0) {
                    uploadImgArr.push(api.NWBDApiAssessImgUrl + res.data.filePath);
                    console.log(uploadImgArr);
                    app.setItem('uploadImgArr', uploadImgArr);
                } else {
    ...
    function createBox($fileInput, file, webUploader) {
        ...
        //-删除图片
        // $('.upload-ul').on('click','.uploadClose',function(){
        //     var index = $(this).parents('li').index();
        //     console.log(index)
        // });

        //绑定取消事件;
        var $diyCancel = $fileBox.find('.uploadClose').one('click', function () {
            var index = $(this).parents('.diyUploadHover').index();
            removeLi($(this).parents('.diyUploadHover'), file_id, webUploader, index - 1);
        });
